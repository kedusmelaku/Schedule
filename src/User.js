...
class User{
    ...
    for (let i = 0; i < this.hours.length; i++){//calculate points from total availability
    ...
    setPriority(level){// sets priority level 0-2
        this.priority = level;
        this.applyPriority();
    }

    applyPriority(){// applies priority to points
        if (this.priority === 1){
            this.points = 0;
